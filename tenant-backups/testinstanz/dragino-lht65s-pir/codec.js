...
function getMyDate(str) {
    ...
    return `${c_Year}-${c_Month.toString().padStart(2, "0")}-${c_Day.toString().padStart(2, "0")} ${c_Hour.toString().padStart(2, "0")}:${c_Min.toString().padStart(2, "0")}:${c_Sen.toString().padStart(2, "0")}`;
}

function datalog(i, input) {
    var Ext = input[6] & 0x0F;
    var bb;
    if ((Ext === 1) || (Ext === 9) || (Ext === 2)) {
        bb = parseFloat(((input[0 + i] << 24 >> 16 | input[1 + i]) / 100).toFixed(2));
    } else if (Ext === 4) {
        var Exti_pin_level = input[0 + i] ? "High" : "Low";
        var Exti_status = input[1 + i] ? "True" : "False";
        bb = Exti_pin_level + Exti_status;
    } else if (Ext === 5 || Ext === 7 || Ext === 8) {
        bb = input[0 + i] << 8 | input[1 + i];
    } else if (Ext === 6) {
        bb = (input[0 + i] << 8 | input[1 + i]) / 1000;
    }

    var cc = parseFloat(((input[2 + i] << 24 >> 16 | input[3 + i]) / 100).toFixed(2));
    var dd = parseFloat((((input[4 + i] << 8 | input[5 + i]) & 0xFFF) / 10).toFixed(1));
    var ee = getMyDate((input[7 + i] << 24 | input[8 + i] << 16 | input[9 + i] << 8 | input[10 + i]).toString(10));
    ...
    switch (poll_message_status) {
        case 0:
            ...
            if (Ext === 0) {
                ...
                decode.ID = str_pad(bytes[2]) + str_pad(bytes[3]) + str_pad(bytes[4]) + str_pad(bytes[5]) +
                    str_pad(bytes[7]) + str_pad(bytes[8]) + str_pad(bytes[9]) + str_pad(bytes[10]);
            }
        ...
        case 1:
            ...
            for (var i = 0; i < bytes.length; i += 11) {
                decode.DATALOG += datalog(i, bytes);
            }
            return { data: decode };
